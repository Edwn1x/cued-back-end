...
=======================
Metrics console for tracking beta performance.
Serves at /admin with real-time stats investors care about:
- Retention cohorts (day 1, 7, 14, 30)
...
Replace the existing admin route in app.py with this.
"""
...
.retention-bar{display:flex;gap:2px;margin-top:4px}
.retention-segment{height:24px;border-radius:4px;display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:600;color:#fff;min-width:30px}

.rating-bars{display:flex;gap:4px;align-items:end;height:60px}
.rating-bar{flex:1;background:var(--accent);border-radius:3px 3px 0 0;min-width:30px;display:flex;align-items:end;justify-content:center;padding-bottom:4px;font-size:10px;color:#fff;font-weight:600}

.msg-feed{max-height:400px;overflow-y:auto}
.msg-item{padding:10px 12px;border-bottom:1px solid var(--border);display:flex;gap:12px;font-size:13px}
.msg-time{color:var(--text3);font-size:11px;min-width:70px;flex-shrink:0}
...
.send-form select,.send-form input,.send-form textarea{background:var(--surface);border:1px solid var(--border);border-radius:6px;color:var(--text);padding:8px;font-size:13px;font-family:inherit}
.send-form select{width:120px}
...
<p class="sub">Beta metrics · Last refreshed: {{ now }}</p>
...
<div class="grid">
  ...
  <div class="stat-card">
    ...
    <div class="stat-sub">by coach</div>
  </div>
  <div class="stat-card">
  ...
  <div class="stat-card">
    ...
    <div class="stat-sub">user replies / coach messages</div>
  </div>
  <div class="stat-card">
    <div class="stat-label">Avg Day Rating</div>
    <div class="stat-val {{ 'green' if avg_rating >= 4 else 'yellow' if avg_rating >= 3 else 'red' }}">{{ avg_rating }}</div>
    <div class="stat-sub">out of 5 ({{ total_ratings }} ratings)</div>
  </div>
  <div class="stat-card">
    <div class="stat-label">Today Active</div>
    <div class="stat-val blue">{{ today_active }}</div>
    <div class="stat-sub">users who texted today</div>
  </div>
</div>
...
<div class="section">
  ...
  <table>
    ...
    <tr>
      <td>Day 1 (responded to first msg)</td>
      <td>{{ d1_responded }}</td>
      <td>{{ d1_rate }}%</td>
...
<div class="section">
  ...
  <table>
    ...
    <tr>
      ...
      <td>{{ u.avg_rating }}</td>
      <td>
        {% if u.days_inactive == 0 %}
        <span class="badge badge-green">ACTIVE TODAY</span>
        {% elif u.days_inactive <= 2 %}
        <span class="badge badge-green">ACTIVE</span>
        {% elif u.days_inactive <= 7 %}
        <span class="badge badge-yellow">QUIET</span>
        {% else %}
        <span class="badge badge-red">INACTIVE {{ u.days_inactive }}d</span>
        {% endif %}
      </td>
    </tr>
    {% endfor %}
    {% if not users %}
    <tr><td colspan="8" class="empty">No users yet. They're coming.</td></tr>
    {% endif %}
  </table>
</div>
...
<div class="section">
  ...
  <div class="msg-feed">
    ...
    <div class="msg-item">
      ...
      <div class="msg-dir">{{ '→' if m.direction == 'out' else '←' }}</div>
      <div class="msg-body">{{ m.body[:120] }}{{ '...' if m.body|length > 120 else '' }}</div>
      <div class="msg-type">{{ m.message_type }}</div>
    </div>
    {% endfor %}
    {% if not recent_messages %}
    <p class="empty">No messages yet. Waiting for Twilio approval.</p>
    {% endif %}
  </div>
</div>
...
<div class="section">
  ...
  <div class="grid" style="grid-template-columns:repeat(3,1fr)">
    <div class="stat-card">
      ...
      <div class="stat-val" style="font-size:24px">${{ twilio_cost }}</div>
      <div class="stat-sub">~$0.015/segment × {{ total_sent + total_received }} msgs</div>
    </div>
    <div class="stat-card">
      <div class="stat-label">Anthropic (API)</div>
      <div class="stat-val" style="font-size:24px">${{ api_cost }}</div>
      <div class="stat-sub">~$0.006/call × {{ total_sent }} calls</div>
    </div>
    <div class="stat-card">
      <div class="stat-label">Total Burn</div>
      <div class="stat-val red" style="font-size:24px">${{ total_cost }}</div>
